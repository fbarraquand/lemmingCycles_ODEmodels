...
y0=[0.25 0.00 0];                            % initial values of state variables vector, % tried 0.1, 0.01, 5
%----------------------------------------------------%

%-------- OPTION HANDLE FOR EVENTS (Check Matlab documentation)-%            
options= odeset('Events',@events_gilg_03,'Reltol',1e-8,'MaxStep',1/365,'NonNegative',[1 2 3]);  % The MaxStep seems to help a lot
% I could use refine as well but somehow this seems more satisfying (though quite slow)
% setting the options for interruption of integration and change in predator density at regular intervals with stoat density multiplied by (1+v) 
...
legend('Nprime','N')
print(figure(1),'-dpdf','-r300','DensitiesLemmingsGilgModel_wStoats')
...
print(figure(2),'-dpdf','-r300','TimeSeriesLogScale_wStoats')
...
xlabel('Time')
print(figure(3),'-dpdf','-r300','TimeSeriesPlusMortality_wStoats')
...
print(figure(5),'-dpdf','-r300','PredatorAbundance_wStoats')
...
xlabel('Prey density')
ylabel('Skua Intake')
%print(figure(6),'-dpdf','-r300','FunctionalResponsesWithUse_wStoats')
