...
function dydt=gilg_func_breakdown(t,y)        % function handle for ODE
...
%------------------------------------------- Adult numbers -----------------------------------------------------------%
if (Nprime>2.0)&&((tmod>t_return_owl_fox)&&(tmod<t_owl_leaves)), P_o = (b_o*(Nprime-2.0))/(Y_o+Nprime-4.0); else P_o = 0.0; end;%%% Owls,  Nprime replaced by N before snowmelt (introduced variable N2 and if condition), corrected
%P_o = 0.0;
...
PredGrowth =  -y(2)*(d_h - Delta*(d_h-d_l)); 
%%% Integration %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
