...
%---------- Lemmings Parameters --------------------%
r_w = 4; % 4 in Gilg et al. 2003, up to 6 - blows up then...
r_s = 0.8; %or 0.4 % or 2 or 0.8. In gilg et al. 2003 0, 0.4, 0.8
...
for (ki=1:km), %%% Loop over the time index of the integrated variables y1,y2,y3
	...
	if (yp1(ki)>2.0)&&((tmod(ki)>t_return_owl_fox)&&(tmod(ki)<t_owl_leaves)), 
		AO(ki) = (b_o*(yp1(ki)-2.0))/(Y_o+yp1(ki)-4.0); % corrected factor 2 
	else 
		AO(ki) = 0.0; 
	end;
...
%------------------------------------- Integrated, cumulative amount predated -----------------------------------------%
dt=y3(2:end)-y3(1:end-1);dt=[dt;dt(end)] % vector of equal size of time increments
IntPredSkua = sum(PredSkua.*dt);
IntPredGen = sum(PredGen.*dt);

% --------------------------------- Find local min and max to compute this over a peak to low --------------------------% 
%[zmax,imax,zmin,imin]= extrema2(y1) %%% Does not work for now

%---- Zoom in on a peak-low, from t=23 to t=25 -------%
%N2325=y1(y3>23&y3<25);
%t2325=y3(y3>23&y3<25);
%PredGen2325=PredGen(y3>23&y3<25);
%[peakN,index_peak]=max(N2325);
%[lowN,index_low]=min(N2325);
%PredGenPeakToLow=PredGen2325(index_peak:index_low);
%dt=t2325((index_peak+1):(index_low+1))-t2325(index_peak:index_low)
%TotPredPeakToLow = sum(PredGenPeakToLow.*dt) %  2.6572
%PercentDeclinePredated = TotPredPeakToLow/(peakN-lowN); % 81%

% some pbs here - replace by findpeaks ? 
